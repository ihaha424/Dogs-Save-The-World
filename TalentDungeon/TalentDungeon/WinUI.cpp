...
WinUI::WinUI()
{
	...
	texture[10] = game::ResourceManager::GetInstance()->LoadTexture("WinUI", "\\Texture\\BackGround\\Win.bmp");
	...
	for (int i = 0; i < 4; i++)
	{
		...
		NumberXPos[i]._position._y = ScreenHeight / 2 - 65.0f;
	}
	...
	for (int i = 4; i < 8; i++)
	{
		...
		NumberXPos[i]._position._y = ScreenHeight / 2 + 40.0f;
	}
	...
	KillBossCount = SharedData::GetInstance()->GetKillBossCount();
	Number[8] = KillBossCount;
	NumberXPos[8] = transform;
	NumberXPos[8]._position._x += 100.0f;
	NumberXPos[8]._position._y = ScreenHeight / 2 + 140.0f;
}
...
void WinUI::Render(HDC& _dc)
{
	...
	for (int i = 4; i < 8; i++)
		texture[Number[i]]->RenderTexture(_dc, NumberXPos[i], RGB(255, 255, 255));
	texture[Number[8]]->RenderTexture(_dc, NumberXPos[8], RGB(255, 255, 255));
}
